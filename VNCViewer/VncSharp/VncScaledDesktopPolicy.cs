	...
	public sealed class VncScaledDesktopPolicy : VncDesktopTransformPolicy
	{
		...
		public override Rectangle AdjustUpdateRectangle(Rectangle updateRectangle)
		{
			...
			adjusted = new Rectangle(
				(int)Math.Round(updateRectangle.X * ScaleFactorX) + ((remoteDesktop.ClientRectangle.Width - scaledSize.Width) / 2),
				(int)Math.Round(updateRectangle.Y * ScaleFactorY) + ((remoteDesktop.ClientRectangle.Height - scaledSize.Height) / 2),
				(int)Math.Round(updateRectangle.Width * ScaleFactorX),
				(int)Math.Round(updateRectangle.Height * ScaleFactorY));
		...
		{
			Size scaledSize = GetScaledSize(remoteDesktop.ClientRectangle.Size);
			src.X = (int)Math.Round((src.X - ((remoteDesktop.ClientRectangle.Width - scaledSize.Width) / 2)) / ScaleFactorX);
			src.Y = (int)Math.Round((src.Y - ((remoteDesktop.ClientRectangle.Height - scaledSize.Height) / 2)) / ScaleFactorY);
			return src;
		}
		...
		private Rectangle GetScaledRectangle(Rectangle rect)
		{
			...
			return new Rectangle((rect.Width - scaledSize.Width) / 2, (rect.Height - scaledSize.Height) / 2,
